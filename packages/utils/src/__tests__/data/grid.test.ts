...
describe('Grid', () => {
  ...
  it('removeNode removes node and cleans up empty rows', () => {
    grid.addNode([0, 0], '0:0', el('a'));
    grid.addNode([0, 1], '0:1', el('b'));
    grid.addNode([2, 0], '2:0', el('c'));
    
    expect(grid.getRow(0)?.index).toBe(0);
    expect(grid.getRow(2)?.index).toBe(2);
    
    grid.removeNode([0, 0]);
    
    expect(grid.getNode([0, 0])).toBeUndefined();
    expect(grid.getNode([0, 1])?.key).toBe('0:1');
    expect(grid.getRow(0)?.index).toBe(0);
    
    grid.removeNode([0, 1]);
    
    expect(grid.getNode([0, 1])).toBeUndefined();
    expect(grid.getRow(0)).toBeUndefined();
    
    expect(grid.firstRow()?.index).toBe(2);
    expect(grid.lastRow()?.index).toBe(2);
    expect(grid.getNode([2, 0])?.key).toBe('2:0');
